...
class TestGenerator(TestCase):
    ...
    def test_load_models_data(self):
        generator = TypingClassesGenerator("odoo/addons/typing_classes_generator/tests")
        with self.mock_get_addon_package_name(), self.mock_get_addon_folder_path():
            generator.generate("fake_module_1")
            from odoo_typing_classes_generator.tests.fake_module_1.typing.base import (
                Model,
            )
            ...
            members = dict(FakeModel1.__dict__)
            # Testing class declaration
            self.assertEqual(
                members["__orig_bases__"],
                (Model[Union[ForwardRef("FakeModel1"), ForwardRef("ResPartner")]],),
            )
            self.assertEqual(
                members["__doc__"],
                (
            ...
            self.assertEqual(a_documented_function_hints["return"], ResCountry)
            ...
            self.assertEqual(
                list(
                    a_static_method_with_default_arguments_signature.parameters.keys()
                ),
