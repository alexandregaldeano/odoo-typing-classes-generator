...
class Generator:
    ...
    def _get_model_data(
        ...
        model_data = ModelData(
            ...
            imports={ImportData.model_import(), ImportData.typing_union_import()},
    ...
    def _get_field_data(
        ...
        if isinstance(field, (fields.Many2one, fields.One2many, fields.Many2many)):
            ...
            if "comodel_name" not in field.args:
                _logger.warning(
                    "Field %s with type %s is invalid.",
                    field_name,
                    field_class,
                )
                return None
    ...
    def _generate_init_file_content(self) -> str:
        ...
        init_file_content += "from .models import (\n    "
        class_names = {
            model_data.unique_class_name
            for model_data in self.merged_models_data_by_odoo_model_name.values()
        }
    ...
    def _generate_class_file_content(self) -> str:
        ...
        class_file_content += ImportData.model_import().serialize() + "\n\n\n"
